...
if /i "%Arc.Order%"=="add" (
	for %%a in (7z rar zip tar bz2 gz xz wim cab) do (
		if /i "%key%"=="%%a" call "%dir.jzip%\Part\Arc_Set.cmd" :Ñ¹Ëõ¸ñÊ½ÇÐ»» %%a
	)
)

if "%key%"=="a" ( if /i "%Arc.Order%"=="add" call "%dir.jzip%\Part\Arc_Set.cmd" :¸ü¸ÄÂ·¾¶
) else if "%key%"=="b" ( if /i "%Arc.Order%"=="add" call "%dir.jzip%\Part\Arc_Set.cmd" :ä¯ÀÀ
) else if "%key%"=="c" ( if /i "%Arc.Order%"=="add" call "%dir.jzip%\Part\Arc_Set.cmd" :¸ü¸ÄÃû³Æ
) else if "%key:~0,1%"=="d" ( for %%a in (rar 7z zip bz2 gz xz cab) do if /i "%Arc.exten%"==".%%a" call "%dir.jzip%\Part\Arc_Set.cmd" :Level %key:~1%
) else if "%key%"=="e" ( if /i "%Arc.Order%"=="add" for %%a in (rar 7z xz) do if /i "%Arc.exten%"==".%%a" call "%dir.jzip%\Part\Arc_Set.cmd" :¹ÌÊµÎÄ¼þ
) else if "%key:~0,1%"=="f" ( for %%a in (rar 7z zip tar bz2 gz xz wim) do if /i "%Arc.exten%"==".%%a" call "%dir.jzip%\Part\Arc_Set.cmd" :·Ö¾íÑ¹Ëõ %key:~1%
) else if "%key:~0,1%"=="g" ( if /i "%Arc.Order%"=="add" for %%a in (rar 7z) do if /i "%Arc.exten%"==".%%a" call "%dir.jzip%\Part\Arc_Set.cmd" :×Ô½âÑ¹  %key:~1%
) else if "%key:~0,1%"=="h" ( for %%a in (rar 7z zip) do if /i "%Arc.exten%"==".%%a" call "%dir.jzip%\Part\Arc_Set.cmd" :Ñ¹Ëõ¼ÓÃÜ %key:~1%
) else if "%key%"=="i" ( if /i "%Arc.Order%"=="add" if /i "%Arc.exten%"==".rar" call "%dir.jzip%\Part\Arc_Set.cmd" :Ñ¹Ëõ°æ±¾.rar
) else if "%key:~0,1%"=="j" ( if /i "%Arc.Order%"=="add" if /i "%Arc.exten%"==".rar" call "%dir.jzip%\Part\Arc_Set.cmd" :Ñ¹Ëõ»Ö¸´¼ÇÂ¼ %key:~1%
...
if defined ×Ô½âÑ¹ (
	for %%A in (
		...
		if "%Arc.exten%"==".rar" if "%×Ô½âÑ¹%"=="%%a" set btn.×Ô½âÑ¹=-sfx"%dir.jzip%\Bin\Sfx\%%b.sfx"
		if "%Arc.exten%"==".7z" if "%×Ô½âÑ¹%"=="%%a" set btn.×Ô½âÑ¹=-sfx"%dir.jzip%\Bin\Sfx\%%c.sfx"
		)
	)
)
