...
if "%key%"=="1" %InputBox% Ñ¹ËõÃÜÂë "%txt_aas.passwd%"
goto :EOF
...
if "%key%"=="1" %InputBox% ·Ö¾íÑ¹Ëõ "%txt_aas.split%" " " "%txt_aas.unit%"
goto :EOF
...
if "%~1"=="-default" (
	set "×Ô½âÑ¹="
	for %%i in (rar 7z) do if "%Archive.exten%"==".%%i" set "×Ô½âÑ¹=a32"
	goto :EOF
)
if "%~1"=="" (
	...
	for %%A in (
		...
		for /f "tokens=1,2,3 delims=/" %%a in ("%%A") do (
			if "%Archive.exten%"==".%%~a" if "%×Ô½âÑ¹%"=="%%~b" set "×Ô½âÑ¹=%%~c" & goto :EOF
		)
	)
)
goto :EOF
...
for /f "delims=" %%i in ("%key1%") do (
	for %%a in (%jzip.spt.write%) do if /i "%%~xi"==".%%a" (
		if /i "%%~xi"==".exe" (
			"%path.editor.7z%" l "%%~i" | findstr /r /c:"^Type = 7z.*" >nul && ( set "Archive.exten=.7z" & set "×Ô½âÑ¹=y" )
			"%path.editor.rar%" l "%%~i" | findstr /r /c:"^Details: RAR.*" >nul && ( set "Archive.exten=.rar" & set "×Ô½âÑ¹=y" )
